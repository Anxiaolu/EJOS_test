...
public class DateUtil {
    ...
    /**
     *
     * @Description: 输入一个整数类型的字符串,然后转换成时分秒的形式输出 例如：输入568 返回结果为：00:09:28
    ...
    /**
     *
     * 功能：<br/> 距离现在几天的时间是多少 获得一个时间字符串，格式为：yyyy-MM-dd HH:mm:ss day 如果为整数，表示未来时间
     * 如果为负数，表示过去时间
     *
     * @author Tony
    ...
    public String getWeek(String sdate) {
        // 再转换为时间
        Date date = this.strToDate(sdate);
        Calendar c = Calendar.getInstance();
        c.setTime(date);
    ...
    public String getNowMonth(String sdate) {
        ...
        String newday = this.getNextDay(sdate, (1 - u) + "");
        return newday;
    }
...
//	  sdate = VeStr.Replace(sdate, "/", "-");
    ...
    public int compareToDateString(String date1, String date2) {
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        int i = 0;
        try {
